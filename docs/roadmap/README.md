...
R-15는 PR #34 merge `main@52fa394`로 코드·Pencil·migration이 반영됐고 Preview/Production DB에 `actual_price_krw`·`set_appointment_actual_price`·R-09 actual revenue 분리를 적용했습니다. 실제 매출은 completed + non-null `actual_price_krw`만 사용하며 snapshot fallback은 없습니다. 기존 예약 backfill은 하지 않았고 authenticated owner/staff UI smoke는 후속 검증입니다. 2026-09-11 금액 입력의 포커스 이탈·브라우저 증감 제거 후속 수정은 `codex/actual-price-input-ux`에서 로컬 검증을 완료했습니다(Node 35 + Jest 47, build, 두 모바일 크기). 해당 수정은 운영 미배포이며 작업 브랜치 Preview에서 owner 로그인과 390×844·360×800 연속 입력·75,000원/0원/빈값 저장·재조회를 검증했습니다. 합성 시술의 금액은 원래 미입력 상태로 복원했습니다. 상세 결과·Pencil micro-fix 예외·실기기 검증 한계는 R-15 문서에 기록했습니다.
