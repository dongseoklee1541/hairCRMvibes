...
2026-07-12 production 작업에서 1~3번의 live 객체 동등성을 다시 확인한 뒤 SQL을 재실행하지 않고 migration history만 `applied`로 repair했습니다. 이어 9번 R-07 migration 한 개만 적용했습니다. 이번 감사에서 live/local migration version 9개가 모두 일치함을 재확인했습니다. 다만 4~8번 live history name은 repair 전 timestamp suffix를 포함하므로 local filename stem까지 같은 `exact match`는 아니며, R-07 row는 name `r07_customer_lifecycle_dedupe`입니다. statements 94개는 release 세션 기록으로 유지합니다.
...
| R-08 | [R-08-service-master.md](./R-08-service-master.md) | Planned (data contract prepared; implementation not started) |
...
- 다음 기능 작업은 R-08 데이터 계약 확인부터 시작하며 R-08 완료 전에는 R-09를 구현하지 않습니다.
- 시작 시 remote `main` SHA를 재확인하고 최신 `origin/main`에서 새 브랜치와 clean worktree를 만듭니다. 오래된 R-07 checkout의 미추적 Playwright/Supabase 산출물은 삭제·이동·stage하지 않습니다.
