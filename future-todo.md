...
| R-02 | Done (live verified) | 예약 상태 check constraint/RPC, 상태 배지, 완료/취소/확정 액션, inline 수정 패널을 구현했고 live RPC/UI/감사 필드 smoke와 모바일 390x844/360x800 screenshot 검증 완료 | PR #37 `aaa168a` Preview에서 인라인 취소 확인 폼·사유 입력·횟수권 복구 검증 완료. PR #37 병합 및 2026-09-11 R-16 Production release에 함께 반영 |
...
| R-04 | Done (live + unit verified) | 화면별 날짜 key/달력/상대 날짜 계산을 `lib/dateTime.js` KST 유틸로 공통화하고 정적 검색, build, 모바일 `/appointments` smoke와 `tests/date-time.test.mjs`의 KST 자정·월말·윤년·연말 경계 테스트를 통과 | 브라우저 timezone 변경을 포함한 전체 화면 time-travel과 실제 기기 장시간 실행은 운영 검증으로 유지 |
...
| R-15 | Done (production deployed; live migration applied; authenticated UI smoke pending) | PR #34 merge `main@52fa394`, implementation `a0f324f` + ignore `96bd4b7`. Vercel Production deployment success, Preview/Production에 `r15_customer_service_price` migration 적용. actual_price 컬럼·RPC·trigger·stats signature 검증 완료. 기존 예약 backfill 0. | 2026-09-11 금액 입력 포커스·증감 제거 후속 수정은 `codex/actual-price-input-ux`에서 로컬 검증 완료(운영 미배포). Preview owner 로그인 및 390×844·360×800 입력·저장·재조회 검증 완료. 상세 근거는 R-15 문서. staff UI·Production authenticated stats·실기기 키보드는 후속 검증. R-16 운영 반영 완료 |
| R-16 | Done (Production 배포·DB 적용·로그인 조회 검증 완료) | PR #37 merge `main@668cd099f397ea9cedcd86d8b216014554bf04aa`, 검토 head `edff562b821fd8a9763c8821a9a0545f1d710356`. Production migration `20260719150346`, Vercel `U8xemVbjjv8NqrfbvwuFezVk8sux`의 canonical 전환 완료. 기존 고객 7건·예약 7건의 개수와 전체 행 해시 보존, RLS/RPC 권한, 로그인 예약·고객·횟수권 조회를 확인했습니다. Node 35/35·race 24/24·build와 최신 Preview owner 상태 전이 검증 통과. | 실기기 IME·설치/standalone·장시간 SW update, Production 횟수권 등록/예약 변경 및 staff 별도 로그인 검증은 후속 운영 검증입니다. 실제 운영 데이터로 쓰기 smoke를 수행하지 않았습니다. |

## 교차 품질 개선 (2026-07-16)

- [AI-slop remediation](./docs/roadmap/ai-slop-remediation-2026-07-16.md): 홈 고객/오늘 예약과 예약 월/일 조회의 실패 상태를 분리하고, 고객 서버 검색의 최소 select·exact count·50개 pagination·특수문자-only 방어, 가격 미설정/무료/유료 구분, 설정 조회 실패 시 저장 잠금, 휴무일 dialog 접근성을 정비했습니다.
...
- `R-16` 고객별 횟수권: 2026-09-11 PR #37 병합, Production DB 적용·canonical 배포·로그인 조회 검증 완료. 실기기 및 운영 쓰기 검증은 후속 항목으로 유지합니다.
