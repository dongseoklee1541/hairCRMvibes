...
| R-02 | Done (live verified) | 예약 상태 check constraint/RPC, 상태 배지, 완료/취소/확정 액션, inline 수정 패널을 구현했고 live RPC/UI/감사 필드 smoke와 모바일 390x844/360x800 screenshot 검증 완료 | PR #37 `aaa168a` Preview에서 인라인 취소 확인 폼·사유 입력·횟수권 복구 검증 완료. Production 반영은 R-16 release와 함께 후속 처리 |
...
| R-16 | In Progress (Preview 취소 검증 완료 · 재확정 보완 로컬 검증 완료) | Draft PR #37, 배포 head `aaa168a`: Preview migration·SQL 역할 계약·owner 합성 UI에서 등록/완료/변경/취소 복구 검증 완료. 2026-09-10 재확정 시 횟수권 누락을 로컬 수정하고 Node 35/35·race 24/24·build·390×844/360×800 비교 검증 통과. | 이번 재확정 수정은 추가 전달 대상으로, 원격 검증은 해당 새 head를 기준으로 확인합니다. commit/push 후 새 Preview 재검증, 최종 PR 검토, Production migration·release는 후속 단계. 실기기 IME·staff 브라우저는 미검증 |
...
- `R-16` 고객별 횟수권: Draft PR #37 및 Preview migration·owner 취소 복구 검증 완료. 재확정 누락 보완은 로컬 검증 완료이며 추가 Git 전달·Preview 검증 단계입니다. 최종 전달·Preview 재검증 후 병합·Production 적용 계획 확정.
