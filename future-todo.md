...
| R-10 | In Progress (local verified; Draft PR merge-blocked) | `origin/main@ec8bff1` 통합 기준 Pencil·DB/RPC·API·UI를 구현하고 12/12 fresh replay·회귀·동시성·rollback, 서버 mock 14건, build·390×844/360×800·PWA/offline·secret 경계를 검증했습니다. 실제 초대·역할 변경은 수행하지 않았습니다. | Draft PR checks와 migration diff를 확인한 뒤, invitation request ledger 범위 승인과 canonical Auth redirect 설정 승인 전에는 merge/live/Production을 진행하지 않음 |
...
## 마지막 업데이트
- 작성일: 2026-07-13
- R-10 local 검증 기록: `origin/main@b225884`에서 구현한 뒤 최신 `origin/main@ec8bff1`을 통합해 Pencil/SQL/API/mobile/PWA를 재검증합니다. 전용 Preview Supabase 격리는 완료됐지만 R-10 Admin 경로의 Preview server secret은 변경·검증하지 않았고 실제 초대·역할 변경도 수행하지 않았습니다. Vercel `SUPABASE_SECRET_KEY`는 `Sensitive / Production`으로 확인했으며 Supabase Auth Site URL `http://localhost:3000`과 Redirect URL 0개, 초대 request ledger 부재를 merge/release blocker로 추적합니다.
