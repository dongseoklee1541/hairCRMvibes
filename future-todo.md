...
| R-10 | In Progress (Draft PR #26; A′ local implementation/verification and new-head PR checks complete; live/release gates remain) | raw email 없는 private HMAC claim ledger와 owner-only claim/settle/reconcile RPC로 Admin invite logical request/active fingerprint당 at-most-once 호출과 `unknown` 자동 재전송 금지를 구현했습니다. PostgreSQL 17 migration 13/13·schema replay·R-07~R-10·동시성·rollback/reapply·catalog parity, 서버 계약 20/20, 무환경/synthetic build, 390×844·360×800, PWA·secret scan을 통과했습니다. A′ 구현 commit `726e1b8` 기준 latest-main merge-tree, Vercel checks와 GitHub `CLEAN/MERGEABLE`도 통과했습니다. local migration은 R-10 두 개이고 Production/Preview는 R-09까지 11개이며 실제 초대·역할 변경은 수행하지 않았습니다. | canonical Auth redirect, `unknown` 조사/종결 runbook과 route 선중지 수단, HMAC 회전 절차, live migration·Production release는 별도 승인으로 진행 |
...
### Phase 3 (P2 확장)
- `R-10` 권한관리 UI(직원 초대/권한변경): In Progress (A′ local implementation/verification and new-head PR checks complete; live/release gates remain)
...
## 마지막 업데이트
- 작성일: 2026-07-14
- R-10 Draft PR #26 기록: `origin/main@b225884`에서 구현한 뒤 최신 `origin/main@a85c3f7`의 R-14 변경까지 실제 merge로 통합했습니다. Pencil 충돌 1건은 R-10·R-12·R-14 node 공존과 layout problem 0건으로 해소했고, A′에서는 `.pen` SHA-1 불변만 확인했으며 현재 Pencil transport로 node/layout 재검증은 blocked입니다. 승인된 A′ private HMAC invitation claim ledger는 동시 동일 request/email의 Admin invite를 winner 1건으로 제한하고 stale/모호한 결과의 자동 재전송을 금지합니다. local fresh/schema replay·회귀·동시성·rollback/reapply·서버 20/20·build·모바일/PWA·secret scan과 구현 commit `726e1b8`의 latest-main merge-tree/Vercel checks/GitHub `CLEAN/MERGEABLE`을 완료했습니다. Production/Preview는 R-09까지 11개 migration이고 R-10 public/private 객체가 없으며 실제 초대·역할 변경은 수행하지 않았습니다. Supabase Auth Site URL `http://localhost:3000`과 Redirect URL 0개, `unknown` 운영 runbook/route 선중지, live migration/Auth 설정/Production release는 별도 gate입니다.
