...
- 2026-07-12 별도 승인 아래 genesis/기존 R-03 세 version의 live 객체 동등성을 다시 확인하고 SQL 재실행 없이 history만 `applied`로 repair했습니다. 이어 R-07 migration 한 개만 적용했습니다. 이번 감사에서 live/local migration version 9개가 모두 일치함을 재확인했으며, 4~8번 live history name에는 repair 전 timestamp suffix가 남아 있어 filename stem까지 동일하다는 의미의 `exact match`로 표현하지 않습니다.
...
- 다음 기능 작업은 R-08 데이터 계약 확인부터 시작하고, R-08 완료 전에는 R-09를 구현하지 않습니다.
- 작업 시작 시 remote `main` SHA를 다시 읽기 전용으로 확인한 뒤 최신 `origin/main`에서 새 작업 브랜치와 clean worktree를 만듭니다. 오래된 R-07 checkout과 `.playwright-cli/`, `output/playwright/**`, `supabase/.temp/`는 삭제·이동·stage하지 않습니다.
