...
begin
  ...
  if tg_op = 'UPDATE' and new.actual_price_krw is not distinct from old.actual_price_krw then
  ...
  if tg_op = 'UPDATE' and current_setting('app.r15_actual_price_rpc', true) is distinct from 'on' then
...
create trigger guard_appointment_actual_price
  ...
  on public.appointments for each row execute function public.guard_appointment_actual_price();
...
begin
  ...
  select p.role into v_role from public.profiles p where p.id = v_actor;
  if v_role is null or v_role not in ('owner', 'staff') then raise exception '실제 금액을 수정할 권한이 없습니다.' using errcode = '42501'; end if;
  if p_appointment_id is null then raise exception '예약 ID는 필수입니다.' using errcode = '22023'; end if;
  if p_actual_price_krw is not null and p_actual_price_krw < 0 then raise exception '실제 금액은 0원 이상이어야 합니다.' using errcode = '22023'; end if;

  select a.* into v_appointment from public.appointments a where a.id = p_appointment_id for update;
  if not found then raise exception '예약을 찾을 수 없습니다.' using errcode = 'P0002'; end if;
  if exists (
    select 1 from public.customers c where c.id = v_appointment.customer_id
      and (c.archived_at is not null or c.merged_into_customer_id is not null or c.anonymized_at is not null)
  ) then raise exception '읽기 전용 고객의 실제 금액은 수정할 수 없습니다.' using errcode = '42501'; end if;
  ...
  if v_appointment.actual_price_krw is not distinct from p_actual_price_krw then
    return query select v_appointment.actual_price_krw, v_appointment.actual_price_updated_at, v_appointment.actual_price_updated_by, v_appointment.actual_price_update_reason;
    return;
  end if;
  ...
  return query update public.appointments a
    set actual_price_krw = p_actual_price_krw, actual_price_update_reason = v_reason
    where a.id = v_appointment.id
    returning a.actual_price_krw, a.actual_price_updated_at, a.actual_price_updated_by, a.actual_price_update_reason;
end;
$$;
...
grant insert (id, customer_id, date, time, service, service_id, duration, duration_minutes, price_snapshot_krw, memo, status, actual_price_krw) on table public.appointments to authenticated;
grant update (customer_id, date, time, service, service_id, duration, duration_minutes, price_snapshot_krw, memo, status, updated_at, cancelled_at, cancelled_by, cancelled_reason) on table public.appointments to authenticated;

drop function if exists public.get_stats_summary(date, date);
create function public.get_stats_summary(p_start_date date, p_end_date date)
returns table (
  period_start date, period_end date, completed_count bigint,
  actual_revenue_krw bigint, paid_actual_completed_count bigint,
  actual_average_ticket_krw bigint, zero_actual_price_completed_count bigint,
  missing_actual_price_completed_count bigint, missing_actual_price_rate numeric,
  missing_actual_price_without_service_count bigint, missing_actual_price_with_service_count bigint,
  booking_snapshot_revenue_krw bigint, booking_snapshot_priced_completed_count bigint,
  booking_snapshot_missing_completed_count bigint, completed_customer_count bigint,
  repeat_customer_count bigint, repeat_rate numeric, service_breakdown jsonb
)
...
begin
  if v_actor is null then raise exception '인증이 필요합니다.' using errcode = '42501'; end if;
  select p.role into v_role from public.profiles p where p.id = v_actor;
  if v_role is null or v_role not in ('owner', 'staff') then raise exception '통계를 조회할 권한이 없습니다.' using errcode = '42501'; end if;
  if p_start_date is null or p_end_date is null then raise exception '시작일과 종료일은 필수입니다.' using errcode = '22023'; end if;
  if p_end_date < p_start_date then raise exception '종료일은 시작일보다 빠를 수 없습니다.' using errcode = '22023'; end if;
  if p_end_date - p_start_date > 365 then raise exception '조회 기간은 최대 366일까지 선택할 수 있습니다.' using errcode = '22023'; end if;
  return query
  with period_completed as materialized (
    select a.customer_id, coalesce(nullif(btrim(a.service), ''), '기타') as service_name, a.service_id, a.actual_price_krw, a.price_snapshot_krw
    from public.appointments a where a.status = 'completed' and a.date >= p_start_date and a.date <= p_end_date
  ), customer_completion_counts as (
    select customer_id, count(*)::bigint as appointment_count from period_completed group by customer_id
  ), summary as (
    select count(*)::bigint as completed_count,
      ...
      case when count(*) filter (where actual_price_krw > 0) = 0 then null else round((sum(actual_price_krw) filter (where actual_price_krw > 0))::numeric / count(*) filter (where actual_price_krw > 0))::bigint end as actual_average_ticket_krw,
    ...
    select count(*)::bigint as completed_customer_count, count(*) filter (where appointment_count >= 2)::bigint as repeat_customer_count from customer_completion_counts
  ), service_metrics as (
    select service_name, count(*)::bigint as completed_count,
      coalesce(sum(actual_price_krw) filter (where actual_price_krw is not null), 0)::bigint as actual_revenue_krw,
      count(*) filter (where actual_price_krw > 0)::bigint as paid_actual_completed_count,
      case when count(*) filter (where actual_price_krw > 0) = 0 then null else round((sum(actual_price_krw) filter (where actual_price_krw > 0))::numeric / count(*) filter (where actual_price_krw > 0))::bigint end as actual_average_ticket_krw,
      count(*) filter (where actual_price_krw is null)::bigint as missing_actual_price_count,
      coalesce(sum(price_snapshot_krw) filter (where price_snapshot_krw is not null), 0)::bigint as booking_snapshot_revenue_krw
    from period_completed group by service_name
  ), top_services as (
    select * from service_metrics order by completed_count desc, actual_revenue_krw desc, service_name asc limit 5
  ), service_json as (
    select coalesce(jsonb_agg(jsonb_build_object('service_name', ts.service_name, 'completed_count', ts.completed_count, 'actual_revenue_krw', ts.actual_revenue_krw, 'paid_actual_completed_count', ts.paid_actual_completed_count, 'actual_average_ticket_krw', ts.actual_average_ticket_krw, 'missing_actual_price_count', ts.missing_actual_price_count, 'booking_snapshot_revenue_krw', ts.booking_snapshot_revenue_krw) order by ts.completed_count desc, ts.actual_revenue_krw desc, ts.service_name asc), '[]'::jsonb) as service_breakdown from top_services ts
  )
  select p_start_date, p_end_date, s.completed_count, s.actual_revenue_krw, s.paid_actual_completed_count, s.actual_average_ticket_krw, s.zero_actual_price_completed_count, s.missing_actual_price_completed_count,
    ...
    s.missing_actual_price_without_service_count, s.missing_actual_price_with_service_count, s.booking_snapshot_revenue_krw, s.booking_snapshot_priced_completed_count, s.booking_snapshot_missing_completed_count,
    cs.completed_customer_count, cs.repeat_customer_count, case when cs.completed_customer_count = 0 then null else round(cs.repeat_customer_count::numeric * 100 / cs.completed_customer_count, 1) end, sj.service_breakdown
  from summary s cross join customer_summary cs cross join service_json sj;
end;
$$;

comment on function public.get_stats_summary(date, date) is 'R-15 KST aggregate. Actual revenue uses completed actual_price_krw only; booking snapshots are separate auxiliary measures and never a fallback.';
revoke all on function public.get_stats_summary(date, date) from public, anon, authenticated;
grant execute on function public.get_stats_summary(date, date) to authenticated;

-- ==========================================
-- R-16: 고객별 횟수권 및 예약 사용 원장
-- ==========================================
...
-- 20260926000000: optional platform event trigger ACL hardening
