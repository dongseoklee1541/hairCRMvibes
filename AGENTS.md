...
- branch 생성/전환, stage·commit·push·PR 생성·merge·배포·원격 서비스 변경은 각각 승인 범위에 포함돼야 합니다. 일반적인 `진행`을 특정되지 않은 운영 DB·권한 변경이나 파괴적 작업으로 확대하지 않습니다.
- 새 고영향 결정, 실질적인 범위 확대, 승인되지 않은 외부·파괴적 변경이 필요하면 그 단계 전에 계획과 영향을 설명하고 승인을 받습니다. 이미 허용된 준비·검증과 독립 작업은 계속합니다.
