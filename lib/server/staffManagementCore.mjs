...
const ERROR_SPECS = {
  ...
  invitation_in_progress: {
    message: '현재 초대 처리가 진행 중입니다. 잠시 기다린 뒤 직원 목록에서 상태를 확인해 주세요.',
    status: 409,
    retryable: false,
  },
  invitation_outcome_unknown: {
    message: '초대 메일 전송 결과를 확인할 수 없습니다. 운영 확인 전에는 같은 이메일로 다시 보내지 마세요.',
    status: 409,
    retryable: false,
  },
  ...
  profile_repair_failed: {
    message: '기존 계정의 직원 권한 등록을 완료하지 못했습니다.',
    status: 502,
    retryable: true,
  },
...
export function isInvitationFeatureEnabled(environment = process.env) {
  return environment?.R10_INVITATIONS_ENABLED === 'true';
}
...
export async function inviteStaffMember(
  ...
  const profiles = (await authorizeOwner()) || [];
  const emailFingerprint = await fingerprintEmail(email);
  const users = (await listAuthUsers()) || [];
  const existingUser = findUserByEmail(users, email);
  const existingProfileRecord = existingUser
    ? profiles.find((profile) => profileUserId(profile) === existingUser.id)
    : null;
  const existingProfile = Boolean(existingProfileRecord);
  ...
  if (existingUser && isEmailConfirmed(existingUser) && existingProfile) {
    try {
      ...
      return {
        ...
        staff: {
          userId: existingUser.id,
          role: existingProfileRecord?.role || 'staff',
          emailMasked: maskEmail(existingUser.email || email),
          emailConfirmed: true,
        },
      };
    } catch {
      // A normal confirmed duplicate has no active invitation ledger to
      // reconcile. Keep the existing duplicate contract in that case.
    }
    throw new StaffManagementError('duplicate_invite');
  }

  const claim = await claimInvitation({ emailFingerprint, requestId });
  if (!claim?.request_id || !claim?.state) {
    throw new StaffManagementError('backend_failure');
  }

  const canonicalRequestId = claim.request_id;
  const claimUserId = claim.auth_user_id || null;

  if (claimUserId && existingUser && claimUserId !== existingUser.id) {
    throw new StaffManagementError('conflict');
  }

  const staffResult = (status, user, provisionResult = {}) => ({
    status,
    inviteState: isEmailConfirmed(user) ? 'accepted' : 'pending',
  ...
  const recoverProvisioning = async (user) => {
    let provisionResult;
    let provisionError;

    try {
      provisionResult = await provisionStaff({
  ...
  if (!claim.acquired) {
    if (claim.state === 'claimed') {
      throw new StaffManagementError('invitation_in_progress');
    }

    if (claim.state === 'unknown') {
      if (existingUser) {
        try {
          const provisionResult = await recoverProvisioning(existingUser);
          if (isEmailConfirmed(existingUser)) {
            return staffResult('repaired', existingUser, provisionResult);
          }
        } catch {
          // The external outcome remains unknown. Recovery is best-effort and
          // must never turn this replay into another email attempt.
        }
      }
      throw new StaffManagementError('invitation_outcome_unknown');
    }

    if (claim.state === 'auth_succeeded') {
      if (!existingUser) {
        throw new StaffManagementError('invitation_in_progress');
      }
    ...
    if (claim.state === 'provisioned') {
      const replayedUser = existingUser || (claimUserId
        ? { id: claimUserId, email, email_confirmed_at: null }
        : null);
      if (!replayedUser) {
        throw new StaffManagementError('backend_failure');
      }
      return staffResult('replayed', replayedUser, {
        next_role: 'staff',
        applied: false,
        replayed: true,
      });
  ...
  const settleClaim = (nextState, authUserId = null, failureCode = null) =>
    settleInvitation({
      requestId: canonicalRequestId,
  ...
  const settleUnknown = async () => {
    try {
      await settleClaim('unknown', null, 'auth_result_unknown');
    } catch {
      // The safe public result is still unknown when the ledger cannot be
      // settled. A later replay remains fail-closed on the existing claim.
    }
  };
  ...
  if (existingUser && isEmailConfirmed(existingUser)) {
    ...
    try {
      ...
      try {
        ...
        // The claim remains fail-closed even if the recovery state cannot be
        // persisted. No Auth email was attempted in this branch.
      }
      throw new StaffManagementError('profile_repair_failed');
    }

    try {
      await settleClaim('provisioned', existingUser.id, null);
    } catch {
      try {
        await reconcileInvitation({
          emailFingerprint,
  ...
  let invitedUser;
  try {
    invitedUser = await inviteUser({ email, redirectTo });
  } catch {
    await settleUnknown();
    throw new StaffManagementError('invitation_outcome_unknown');
  }
  ...
  if (
    !invitedUser?.id ||
    !UUID_PATTERN.test(invitedUser.id) ||
    typeof invitedUser?.email !== 'string' ||
    invitedUser.email.trim().toLowerCase() !== email ||
    (existingUser && existingUser.id !== invitedUser.id)
  ) {
    await settleUnknown();
    throw new StaffManagementError('invitation_outcome_unknown');
  }
  ...
  try {
    await settleClaim('auth_succeeded', invitedUser.id, null);
  } catch {
    try {
      const provisionResult = await recoverProvisioning(invitedUser);
      return staffResult(existingUser ? 'reinvited' : 'invited', invitedUser, provisionResult);
    } catch {
      throw new StaffManagementError('invitation_outcome_unknown');
    }
  }
  ...
  try {
    ...
    try {
      await settleClaim('auth_succeeded', invitedUser.id, 'profile_provision_failed');
    } catch {
      // The ledger already records Auth success when the first settlement
      // succeeded, so retry remains an email-free provisioning repair.
    }
    throw new StaffManagementError('partial_failure', {
      result: { status: 'pending', inviteState: 'pending' },
    });
  ...
  try {
    await settleClaim('provisioned', invitedUser.id, null);
  } catch {
    try {
      await reconcileInvitation({
        emailFingerprint,
...
export async function handleInvitationPost(
  request,
  {
  ...
  try {
    ...
    if (!isInvitationFeatureEnabled(environment)) {
      return errorResponse(new StaffManagementError('invitation_maintenance'), {
        privateNoStore: true,
      });
    }
...
export function toStaffManagementError(error, fallbackCode = 'backend_failure') {
  ...
  if (error?.code === '55000') {
    ...
    if (includesMessage(error, '초대 처리가 이미 진행')) {
      return new StaffManagementError('invitation_in_progress');
    }
  }
...
export function jsonResponse(body, status = 200, headers = RESPONSE_HEADERS) {
...
export function errorResponse(error, { privateNoStore = false } = {}) {
  ...
  return jsonResponse(
    body,
    safeError.status,
    privateNoStore ? PRIVATE_NO_STORE_HEADERS : RESPONSE_HEADERS,
  );
}
